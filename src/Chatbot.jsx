...
const Chatbot = () => {
  ...
  const handleAsk = async (customPrompt) => {
    ...
    try {
      ...
      console.error(error);
      setResponse(prev => [...prev, { sender: "AI", text: "Error connecting to IQGalaxy Assistant." }]);
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    if (open) {
      const greeting = "Hello, welcome to IQGalaxy. How can I help you?";
      ...
      {open && (
        <div className="fixed bottom-20 right-6 w-80 md:w-96 lg:w-[400px] h-[500px] md:h-[550px] bg-[#0a0f16]/95 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-4 text-white z-50 flex flex-col">
          ...
          <div className="flex justify-between items-center border-b border-white/10 pb-2 mb-3">
            <h2 className="font-semibold text-lg text-cyan-400">IQGalaxy Assistant</h2>
            <button
